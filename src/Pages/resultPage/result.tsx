...
const toIsoDate = (s?: string | null): { iso: string | null; raw: string | null } => {
  ...
  return d ? { iso: d.toISOString(), raw: trimmed } : { iso: null, raw: trimmed };
};

const deriveSuccess = (params: URLSearchParams): boolean => {
  const code = (params.get("code") || params.get("respCode") || "").toLowerCase();
  ...
  if (status === "success" || status === "succeeded" || status === "paid" || status === "completed") return true;
...
const ResultPage: React.FC = () => {
  ...
  const data: ResultData = useMemo(() => {
    ...
    const amount = toNumber(params.get("amount") || params.get("total") || params.get("price"));
    const { iso: paidAt, raw: rawPaidAt } = toIsoDate(
      params.get("paidAt") || params.get("time") || params.get("txnTime") || params.get("timestamp")
    );
    ...
    return {
      ...
      orderId: params.get("orderId") || params.get("order_no") || params.get("billId"),
      transactionId: params.get("transactionId") || params.get("txnId") || params.get("trans_id"),
    ...
    <div style={{ maxWidth: 720, margin: "24px auto", padding: "16px" }}>
      ...
      >
        ...
        <div>
          <div style={{ fontWeight: 700, fontSize: 18 }}>
            {data.success ? "Thanh toán thành công" : "Thanh toán thất bại / không xác định"}
          </div>
          {data.message ? <div style={{ opacity: 0.8 }}>{data.message}</div> : null}
        </div>
      </div>
      ...
      <div style={{ marginTop: 16, border: "1px solid #e5e7eb", borderRadius: 12, padding: 16 }}>
        ...
        <dl style={{ display: "grid", gridTemplateColumns: "180px 1fr", rowGap: 8 }}>
          <dt>Mã đơn hàng</dt><dd>{data.orderId || "-"}</dd>
          <dt>Mã giao dịch</dt><dd>{data.transactionId || "-"}</dd>
          <dt>Đơn vị chấp nhận</dt><dd>{data.merchantId || "-"}</dd>
          <dt>Phương thức</dt><dd>{data.method || "-"}</dd>
          <dt>Số tiền</dt><dd>{data.amount != null ? data.amount.toLocaleString() : "-"} {data.currency || ""}</dd>
          <dt>Thời gian thanh toán</dt><dd>{data.paidAt || data.rawPaidAt || "-"}</dd>
          <dt>Trạng thái</dt><dd>{data.status || (data.success ? "success" : "-")}</dd>
          <dt>Mã phản hồi</dt><dd>{data.code || "-"}</dd>
          <dt>Hash (fragment)</dt><dd>{data.hash || "-"}</dd>
        </dl>
      </div>
